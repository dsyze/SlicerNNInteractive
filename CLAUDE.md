...
**Tri-planar multi-series mode** (`dev_docs/triplanar_multiseries.md`) -- `cbTriPlanarMode` ties the above together for three co-registered series: auto-assigns axial/sagittal/coronal series to the Red/Yellow/Green views (`_classify_series_orientation`, `_setup_triplanar_views`), routes each interaction to the series shown in the view it was made in (`_view_for_ras_point` + the `_active_inference_volume_override` honored by `get_inference_volume_node`), and after every interaction direction-weighted-fuses the three directions (`_fuse_series_results` blurs each series only along its thick axis via `_series_anisotropic_sigma`) and shows the result (`_maybe_autofuse`). In this mode multi-view lasso accumulation is bypassed (each view is a different series).
